...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/ymlsn/soga/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 soga 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 soga 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 soga 最新版本：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://github.com/ymlsn/soga/releases/download/${last_version}/soga-cracked-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/ymlsn/soga/releases/download/${last_version}/soga-cracked-linux64.tar.gz"
    ...
    if [[ ! -f /etc/soga/soga.conf ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}soga 可能启动失败，请稍后使用 soga log 查看日志信息，若无法启动，则可能更改了配置格式，请前往 wiki 查看：https://github.com/ymlsn/soga/wiki${plain}"
        fi
    fi
    ...
    curl -o /usr/bin/soga -Ls https://raw.githubusercontent.com/ymlsn/soga/master/soga.sh
